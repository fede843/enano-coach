...
| Normalizer sleep | Creates non-overlapping intervals; unknown/gap coverage uses generic `sleeping` rather than inventing deep/light/rem. | `../gadgetbridge-ow/src/gadgetbridge_ow/normalizer.py:174-242`; `../gadgetbridge-ow/tests/test_normalizer.py:171-217` | `fork_extension` | [PROPOSED] | The product choice between `sleeping` and `unknown` remains pending. |
